        ...
        try {
            if ($id) {
                ...
                if($query){
                    ...
                    $query->images = json_encode($request->images, $jsonFlags);
                    $query->description = json_encode($request->description, $jsonFlags);
                    $query->content = json_encode($request->content, $jsonFlags);
                    $query->size = json_encode($request->size, $jsonFlags);
                    $query->variant = json_encode($request->variant, $jsonFlags);
                    $query->category = $request->category;
                    $query->type_two = $request->type_two;
                    $query->preserve = json_encode($request->preserve, $jsonFlags);
                    $query->tags = json_encode($request->tags, $jsonFlags);
                ...
                $query->ingredient = $request->ingredient;
                $query->type_cate = $request->type_cate;
                $query->species = json_encode($request->species, $jsonFlags);
                $query->origin = $request->origin;
                $query->thickness = $request->thickness;
                $query->status_variant = $request->status_variant;
                $query->hang_muc = $request->hang_muc;
